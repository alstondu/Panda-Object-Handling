...
The project contains three tasks, and each of them are defined here. There
...
that will be received and respondd to (by solving the task).

Currently, tasks may spawn with random box positions and colours. Task parameters can be edited:
...
from Panda_world_spawner_lib.Panda_world_spawner import *
from panda_world_spawner.srv import TaskSetup, TaskSetupResponse
from panda_world_spawner.srv import Task1Service, Task2Service, Task3Service
import numpy as np
...
class World(object):
  ...
  def __init__(self):
    """
    World managing class for the project
    """
...
class Task(object):
  ...
  def __init__(self, mode='project', validation_scenario=0):
    """
...
# ----- Class definitions for the project tasks ----- #
...
class Task1(Task):
  ...
  def __init__(self, mode='project', validation_scenario=0):
    """
    Task class, initialise either in 'project' mode (normal, no validation
...
if __name__ == "__main__":
  ...
  # create the world and run the project /task service
  rospy.init_node('project_wrapper')
